...
const SolicitudesServiciosRechazadas: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSolicitudesServicios = async () => {
      try {
        ...
        // Filtrar las solicitudes por estado 'Rechazada'
        const filteredSolicitudes = result.data.solicitudesServicios.filter((solicitud: SolicitudServicio) =>
          solicitud.id_solicitud_estado.estado === 'Rechazada'
        );
        setSolicitudesServicios(filteredSolicitudes);
        setTotalPages(result.data.totalPages); // Asumir que el servidor devuelve el total de páginas
        setLoading(false);
      } catch (error) {
...
export default SolicitudesServiciosRechazadas;
